...
def export_csv(source, date_published, date_modified, sponsor, title, introduction, body):
    output = open('voon.csv', 'w', encoding='utf-8')
...
class scraper:
    def __init__(self, id):
        self.url = "https://ieeexplore.ieee.org/abstract/document/" + id
...
if __name__ == '__main__':
    ...
    # with open('ads.txt', 'r+') as file_id:
    str_id = "9240634";
    id = scraper(str_id[:-1])
    export_csv('nu.nl', id.get_date_published(), id.get_date_modified(), id.get_sponsor(), id.get_title(), id.get_introduction(), id.get_body())
        # for id in file_id:
        #     if id != "":
        #         str_id = str(id)
        #         id = scraper(str_id[:-1])
        #         if ((id.get_date_published() != "") and (len(id.get_date_published()) < 50)) and (id.get_sponsor() != '"Tijdschrift.nl"')and (id.get_sponsor() != '"NUshop advertorial"'):
        #             export_csv('nu.nl', id.get_date_published(), id.get_date_modified(), id.get_sponsor(), id.get_title(), id.get_introduction(), id.get_body())
